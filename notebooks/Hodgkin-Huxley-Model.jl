...
# ╔═╡ dc2d1d02-52de-48de-8224-782a7600de24
using DifferentialEquations

# ╔═╡ 261826dd-ba4f-47bf-a271-dd6a98e0b709
using Plots

# ╔═╡ 008e4887-dcd2-42de-b0b6-a7dbdd42361c
# 1. Parameter Structure
struct HHParameters
    C_m::Float64 # Membrane capacitance (µF/cm²)
    g_Na::Float64 # Maximum conductance of Na⁺ channel (mS/cm²)
    g_K::Float64 # Maximum conductance of K⁺ channel (mS/cm²)
    g_L::Float64 # Maximum conductance of leak channel (mS/cm²)
    E_Na::Float64 # Na⁺ reversal potential (mV)
    E_K::Float64 # K⁺ reversal potential (mV)
    E_L::Float64 # Leak reversal potential (mV)
    E_syn::Float64 # Synaptic reversal potential (mV)
    tau_syn::Float64 # Synaptic time constant (ms)
    W_syn::Float64 # Synaptic weight
end

# ╔═╡ 4a27c6c6-2014-4f6f-aebb-297d41ec6a0d
# 2. Gating Functions
αn(V) = abs(V + 55.0) < 1e-9 ? 0.1 : (0.01 * (V + 55.0)) / (1.0 - exp(-(V + 55.0) / 10.0)) # K⁺ activation

# ╔═╡ 630b50c6-5290-4bd4-ace7-01b4925acb59
βn(V) = 0.125 * exp(-(V + 65.0) / 80.0) # K⁺ deactivation

# ╔═╡ af5bb52d-7243-41e0-abd3-727e5ef575ce
αm(V) = abs(V + 40.0) < 1e-9 ? 1.0 : (0.1 * (V + 40.0)) / (1.0 - exp(-(V + 40.0) / 10.0)) # Na⁺ activation

# ╔═╡ 737113e4-29d2-4419-a131-838f88724987
βm(V) = 4.0 * exp(-(V + 65.0) / 18.0) # Na⁺ deactivation

# ╔═╡ 7b469332-cba9-40fe-bf9c-1237f2e5e9b9
αh(V) = 0.07 * exp(-(V + 65.0) / 20.0) # Na⁺ inactivation

# ╔═╡ 64ee5c2e-5533-4047-8af3-e89d90d20bbe
βh(V) = 1.0 / (1.0 + exp(-(V + 35.0) / 10.0)) # Na⁺ deinactivation

# ╔═╡ 7b46df5e-def7-4381-8b93-1af8161d9b1e
# 3. The Integrated System
function hh_synapse_system!(du, u, p, t)
    V, n, m, h, g_syn = u

    # Standard Ionic Currents
    I_Na = p.g_Na * m^3 * h * (V - p.E_Na)
    I_K = p.g_K * n^4 * (V - p.E_K)
    I_L = p.g_L * (V - p.E_L)

    # New Synaptic Current (Excitatory if E_syn > V_rest)
    I_syn = g_syn * (V - p.E_syn)

    # Derivatives
    du[1] = (-(I_Na + I_K + I_L + I_syn)) / p.C_m  # dV/dt
    du[2] = αn(V) * (1.0 - n) - βn(V) * n          # dn/dt
    du[3] = αm(V) * (1.0 - m) - βm(V) * m          # dm/dt
    du[4] = αh(V) * (1.0 - h) - βh(V) * h          # dh/dt
    du[5] = -g_syn / p.tau_syn                     # dg_syn/dt (Exponential Decay)
end

# ╔═╡ 4e8a74db-f05f-445d-8f6a-4579784a0d2b
# 4. Callbacks for Synaptic "Kicks"
# Add synaptic weight to conductance when the callback triggers
function kick!(integrator)
    integrator.u[5] += integrator.p.W_syn
end

# ╔═╡ d11d9b95-9f06-4d1d-b655-8a3da06b8d98
# We set triggers at 10ms and 15ms to test summation
cb1 = PresetTimeCallback(10.0, kick!)

# ╔═╡ 5ba13dd2-6c4b-45d2-af7a-5d013bff7ac9
cb2 = PresetTimeCallback(15.0, kick!)

# ╔═╡ e0734f86-8270-4ed8-85c8-a16c2becbbd5
cb_set = CallbackSet(cb1, cb2)

# ╔═╡ 00be2f84-8591-41f4-90f6-fde7b2d6ab05
# 5. Simulation Setup
u0 = [-65.0, 0.317, 0.05, 0.6, 0.0]

# ╔═╡ 9e7b2a43-e4a6-42e6-8ffc-a77596e01dc7
tspan = (0.0, 40.0)

# ╔═╡ 0b4f88eb-6456-4328-a142-95b701430c02
# E_syn = 0.0 for excitation; try -80.0 for inhibition!
params = HHParameters(1.0, 120.0, 36.0, 0.3, 50.0, -77.0, -54.4, 0.0, 2.0, 0.5)

# ╔═╡ f1656d41-155b-47e7-9881-3a2fd3745a2a
prob = ODEProblem(hh_synapse_system!, u0, tspan, params)

# ╔═╡ 4f7ecf0f-b07a-4c17-bf26-c9ddce3a9403
# PresetTimeCallback automatically ensures the solver hits these times
sol = solve(prob, callback=cb_set)

# ╔═╡ 98d423c4-5c9b-451b-a8e6-f66969c80fe5
# 6. Plotting
p1 = plot(sol, idxs=1, title="Membrane Potential (V)", ylabel="mV", label="V")

# ╔═╡ 90b8a275-ef9b-4f90-bc17-f8e7cda61e6b
p2 = plot(sol, idxs=5, title="Synaptic Conductance (g_syn)", ylabel="mS/cm²", label="g_syn", color=:red)

# ╔═╡ 19ac91d1-03aa-4b50-9a6f-8b11730582ee
HH_Plot = plot(p1, p2, layout=(2, 1), size=(800, 600))

# ╔═╡ ac141dca-aa4e-4b52-af73-0411726bf2a1
display(HH_Plot)
...
manifest_format = "2.0"
project_hash = "70b2bee5613091b5307fbee23d00e2b860a42746"
...
git-tree-sha1 = "7289c72e0db3199f3c5b4660395c737476d2b97b"
...
git-tree-sha1 = "146c8603444eca98b501441a5e2a412b6719209a"
...
git-tree-sha1 = "78492306505abc7d9a78a63d52d577ec1a994c8d"
...
git-tree-sha1 = "54c15875d52718e5f53b3b51836f408cc6eeb9fc"
...
git-tree-sha1 = "b174e70ade85308119a570caac49edeb665991c0"
...
git-tree-sha1 = "83047a5d6a285c9acf1484ab47158ff42235a9a6"
...
git-tree-sha1 = "651bcb90aa6927beacbcb441076f0a0feec3b4f5"
...
git-tree-sha1 = "a29cf26cd3314f0be0bc04d42c6d0170dfa65d68"
...
git-tree-sha1 = "d56195f5c82bfdfd785f403d51a5a47a7ab6d0ec"
...
git-tree-sha1 = "bb5b03566c0260da0b21c07fb6d00f6ec3b5e795"
...
git-tree-sha1 = "dbf07c2981b44bd4c5cbb6180819f7a3a43ff1d2"
...
git-tree-sha1 = "c4a9880629868e6eafe5aa902c8e4098b8936aa9"
...
git-tree-sha1 = "10947da4f1b823ed43890bddda7e8d19dde89e2e"
...
git-tree-sha1 = "6a5a2469f691845c66de9b4872904e387efee629"
...
[[deps.TestEnv]]
deps = ["Pkg"]
git-tree-sha1 = "c5bdc42698a72f9d0bd79a38b464c4265ae7d7fd"
